...
public class EbnfToJava {
	...
	private static class EbnfTranslator {
		...
		String translate(NonterminalNode grammar, LinkedList<String> ruleComments) {
			...
			int i = 0;
			for (TreeNode child : grammar.getChildren()) {
				NonterminalNode rule = (NonterminalNode) child; // Instance is guaranteed by the Parser

				String identifier = rule.getTerminalChild(0).getToken().getText();
...
//				javaCode.append(");\n}\n");
		...
		void a(TreeNode node) {
			if (node instanceof TerminalNode) {
				...
				switch (terminalNode.getValue()) {
					case IDENTIFIER:
						javaCode.append("this::").append(toCase1(terminalNode.getToken().getText()));
						break;
					case STRING_LITERAL:
						boolean found = false;
						for (HashMap<String, Terminal> map : TextConstants.ALL_MAPS) {
							Terminal value = map.get(terminalNode.getToken().getText());
							if (value != null) {
								found = true;
						...
						if (!found) {
							javaCode.append("() -> identifier(\"").append(terminalNode.getToken().getText()).append("\")");
						}
						break;
				}
			} else {
				...
				switch (nonterminalNode.getValue()) {
					case OPTIONAL:
						...
						a(nonterminalNode.getChildren().get(0));
						break;
					case REPEATING:
	...
	private static class EbnfParser {
		...
		boolean accept(NonterminalNode parent, Terminal value) {
			...
			if (accepted) {
				parent.getChildren().add(new TerminalNode(currentToken));
