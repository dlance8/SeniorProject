...
public class ParserV2 {
	...
	private boolean lookAhead(int k, Terminal value) {
		return parsing && currentIndex + k < tokens.size() && tokens.get(currentIndex).getValue() == value;
	}
...
//		return accepted;
	...
	private boolean methodInvocation() {
		return accept(Nonterminal.METHOD_INVOCATION, () -> acceptAny(
			...
			() -> acceptAll(this::expressionName, () -> accept(Terminal.DOT), () -> acceptOptional(this::typeArguments), this::identifier, () -> accept(Terminal.OPEN_PARENTHESIS), () -> acceptOptional(this::argumentList), () -> accept(Terminal.CLOSE_PARENTHESIS)),
			() -> {
				boolean any = false;
				while (acceptAll(this::identifier, () -> accept(Terminal.DOT)))
					if (!any) any = true;
				return any && acceptAll(() -> acceptOptional(this::typeArguments), this::identifier, () -> accept(Terminal.OPEN_PARENTHESIS), () -> acceptOptional(this::argumentList), () -> accept(Terminal.CLOSE_PARENTHESIS));
			},
	...
	private boolean relationalExpression() {
		return accept(Nonterminal.RELATIONAL_EXPRESSION,
			() -> acceptAny(
				...
				() -> acceptAll(
					() -> {
						final boolean accepted = shiftExpression();
						return accepted;
					},
					() -> {
						final boolean accepted = acceptRepeating(
							() -> {
								final boolean acceptedAll = acceptAll(
									() -> acceptAny(
